...
class WinAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 4, 3, 5, 1).contiguous().view(b, h//self.window_size, w//self.window_size, self.window_size * self.window_size, c).cuda()
    ...
    def forward(self, x):
        ...
        for r in range(x.shape[1]):  # 相应的行
            for c in range(x.shape[2]):  # 相应的列
                ...
                dis_x = torch.tensor([(h-c)**2 for h in range(x.shape[2])]).cuda()  # (b, w)
                dis_y = torch.tensor([(w-r)**2 for w in range(x.shape[1])]).cuda()  # (b, h)
                ...
                new_value_full[:, r, c, :] = torch.sum(atten_x.unsqueeze(dim=-1) * value_full[:, r, :, :] + atten_y.unsqueeze(dim=-1) * value_full[:, :, c, :], dim=-2)
